...
var logger = log4js.getLogger('HRMS-Salary-Calculations-Controller');
logger.level = 'All';
...
var SDServices = require("../services/SalaryDetails/SalaryCalculationServices");
...
router.get('/getYearMonthPeriod', function (req, res, next) {
    SDServices.getYearMonthPeriod().then(salaryCycles => {
        res.json(salaryCycles)
    }).catch((err) => {
        logger.error("Err when get available Salary Cycles: " + err);
        res.json({
            status: 500,
            data: [],
            message: "Err when get OT Data: " + err
...
router.get('/finalizeSalaryCalData', function (req, res, next) {
    SDServices.finalizeSalaryCalData().then((resdata) => {
        res.json(resdata)
        res.end();
    }, (err) => {
        logger.error(err);
...
router.get('/reSyncData', function (req, res, next) {
    SDServices.syncData().then(srres => {
        res.json(srres);
        res.end();
    })
})
