...
EmpDeskAccount.getAvailableCalendarYear = function() {
  return new Promise(function(rel, rej) {
    ...
    sequelize
      .query("select configDoc from ConfigDocs where configKey='ConfigData'", { type: sequelize.QueryTypes.SELECT })
      .then(configDoc => {
    ...
    sequelize
      .query("SELECT DISTINCT LEFT(salaryCycle,4) as years FROM SalaryCycles", { type: sequelize.QueryTypes.SELECT })
      .then(
        years => {
...
EmpDeskAccount.getEmpDeskAccountByYear = function(calendarYear) {
  return new Promise(function(rel, rej) {
    ...
    sequelize
      .query(
        "SELECT empId,name,workerCategory,department, jobrole, salaryCycle, yingfagongzi,jibengongzi,yicixingjiangjin from SalaryDetails where salaryCycle>=:startSalaryCycle and salaryCycle<=:endSalaryCycle",
        {
          replacements: { startSalaryCycle: startSalaryCycle, endSalaryCycle: endSalaryCycle },
          ...
          for (let i = 0; i < saldata.length; i++) {
            ...
            for (let k = 0; k < empaccount.length; k++) {
              if (empaccount[k].empId === saldata[i].empId) {
                ...
                //empaccount[k][samonth + 'Xiaoji'] = saldata[i].yingfagongzi;
                empaccount[k][samonth + "Gongzi"] = saldata[i].jibengongzi;
                empaccount[k][samonth + "Jiangjin"] = keepTwoDecimalFull(
                ...
                empaccount[k].JANTODECXiaoji = keepTwoDecimalFull(
                  parseFloat(empaccount[k].JANTODECXiaoji) + parseFloat(empaccount[k][samonth + "Xiaoji"])
                );
                empaccount[k].JANTODECGongzi = keepTwoDecimalFull(
                  parseFloat(empaccount[k].JANTODECGongzi) + parseFloat(empaccount[k][samonth + "Gongzi"])
                );
                empaccount[k].JANTODECJiangjin = keepTwoDecimalFull(
                  parseFloat(empaccount[k].JANTODECJiangjin) + parseFloat(empaccount[k][samonth + "Jiangjin"])
                );

                zongji[samonth + "Xiaoji"] = keepTwoDecimalFull(
                  parseFloat(zongji[samonth + "Xiaoji"]) + parseFloat(empaccount[k][samonth + "Xiaoji"])
                );
                zongji[samonth + "Gongzi"] = keepTwoDecimalFull(
                  parseFloat(zongji[samonth + "Gongzi"]) + parseFloat(empaccount[k][samonth + "Gongzi"])
                );
                zongji[samonth + "Jiangjin"] = keepTwoDecimalFull(
                  parseFloat(zongji[samonth + "Jiangjin"]) + parseFloat(empaccount[k][samonth + "Jiangjin"])
                );

                zongji.JANTODECXiaoji = keepTwoDecimalFull(
                  parseFloat(zongji.JANTODECXiaoji) + parseFloat(empaccount[k][samonth + "Xiaoji"])
                );
                zongji.JANTODECGongzi = keepTwoDecimalFull(
                  parseFloat(zongji.JANTODECGongzi) + parseFloat(empaccount[k][samonth + "Gongzi"])
                );
                zongji.JANTODECJiangjin = keepTwoDecimalFull(
                  parseFloat(zongji.JANTODECJiangjin) + parseFloat(empaccount[k][samonth + "Jiangjin"])
                );
            ...
            if (!isEmpaSAExist) {
              let newemp = createEmpAccount(
                saldata[i].empId,
                saldata[i].name,
                saldata[i].workerCategory,
                saldata[i].department,
                saldata[i].jobrole
              );
              let samonth = translateMonthNumToMonth(saldata[i].salaryCycle.substring(4, 6));
              ...
              zongji[samonth + "Xiaoji"] = keepTwoDecimalFull(
                parseFloat(zongji[samonth + "Xiaoji"]) + parseFloat(newemp[samonth + "Xiaoji"])
              );
              zongji[samonth + "Gongzi"] = keepTwoDecimalFull(
                parseFloat(zongji[samonth + "Gongzi"]) + parseFloat(newemp[samonth + "Gongzi"])
              );
              zongji[samonth + "Jiangjin"] = keepTwoDecimalFull(
                parseFloat(zongji[samonth + "Jiangjin"]) + parseFloat(newemp[samonth + "Jiangjin"])
              );

              zongji.JANTODECXiaoji = keepTwoDecimalFull(
                parseFloat(zongji.JANTODECXiaoji) + parseFloat(newemp.JANTODECXiaoji)
              );
              zongji.JANTODECGongzi = keepTwoDecimalFull(
                parseFloat(zongji.JANTODECGongzi) + parseFloat(newemp.JANTODECGongzi)
              );
              zongji.JANTODECJiangjin = keepTwoDecimalFull(
                parseFloat(zongji.JANTODECJiangjin) + parseFloat(newemp.JANTODECJiangjin)
              );
